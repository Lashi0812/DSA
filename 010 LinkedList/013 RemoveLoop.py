"""
    Given the Linked list with the loop , task is to remove that loop.
"""
...
class LinkedList:
    ...
    def removeLoop(self)-> Self:
        loop_start_point = self.whereLoopStart()
        node = loop_start_point
        
        while node is not None and node.next != loop_start_point:
            node = node.next
