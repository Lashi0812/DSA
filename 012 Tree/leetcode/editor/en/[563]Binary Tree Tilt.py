...
#  
# 
#  Related Topics Tree Depth-First Search Binary Tree 👍 1857 👎 2002
...
class Solution(object):
    def findTilt(self, root):
        ...
        :rtype: int
        """
        _, _, ans = self._postorder(root)
        return ans

    def _postorder(self, node, sums=0, diff=0, tilt_sum=0):
        if node is None:
            return sums, diff, tilt_sum
        sum_l, diff_l, tilt_sum = self._postorder(node.left, sums, diff, tilt_sum)
        sum_r, diff_r, tilt_sum = self._postorder(node.right, sums, diff, tilt_sum)
        return sum_r + sum_l + node.val, abs(sum_r - sum_l), tilt_sum + abs(sum_r - sum_l)
